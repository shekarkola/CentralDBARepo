USE [DBAClient]
GO
/****** Object:  StoredProcedure [dbo].[sp_blocked_process_report_viewer]    Script Date: 10/10/2024 12:03:40 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN
		...
		INSERT #ReportsXML(blocked_ecid,blocked_spid,blocking_ecid,blocking_spid,
			monitorloop,bpReportXml,endTime,  duration, databasename)
		SELECT blocked_ecid, blocked_spid, blocking_ecid, blocking_spid,
			COALESCE(monitorloop, CONVERT(nvarchar(100), eventDate, 120), cast(newid() as nvarchar(100))),
			bpReportXml,eventDate, duration, databasename
		...
		CROSS APPLY (
			SELECT 
				...
				duration = event_file_value_xml.[xml].value('(event[@name="blocked_process_report"]/data[@name="duration"]/value)[1]', 'bigint') / 1000,
				databasename = event_file_value_xml.[xml].value('(event[@name="blocked_process_report"]/data[@name="database_name"]/value)[1]', 'nvarchar(200)')
		) as bpReports
		CROSS APPLY (
			SELECT 
				monitorloop = bpReportXml.value('(//@monitorLoop)[1]', 'nvarchar(100)'),
