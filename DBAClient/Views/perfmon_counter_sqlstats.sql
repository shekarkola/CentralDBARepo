...
CREATE OR ALTER VIEW perfmon_counter_sqlstats AS
---- SQL Stats View:
with sqlstats as (
select instance_fullname, sample_time, counter_name
		--,LAG(cntr_value) OVER (PARTITION BY instance_fullname, object_name, counter_name, instance_name ORDER BY sample_time) as prev_countervalue
		--cntr_value
		,cntr_value - 
